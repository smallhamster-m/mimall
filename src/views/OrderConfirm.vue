<template>
  <div class="order-confirm">
    ...
    <modal
      ...
      btnType="3"
      :isShow="showDelModal"
      @submit="submitDel(checkedItem)"
      @close="showDelModal = false"
      ><template v-slot:body><p>您确认要删除此地址吗？</p></template></modal
    >
    <modal
      title="修改/新增 地址"
      btnType="3"
      :isShow="showEditModal"
      @submit="submitEdit(checkedItem)"
...
export default {
  ...
  data() {
    return {
      currentIndex: 0, //当前收货地址选中索引
      showDelModal: false, //是否显示 删除 的对话框
      showEditModal: false, //是否显示 修改/新增 的对话框
      userAction: '',//用户行为 0：编辑 1：新增 
      cartList: [], //购物车需要结算的商品
      cartTotalPrice: 0, //购物车需要结算商品的总价
      count: 0, //购物车需要结算商品数量
      addressList: [], //收货地址列表
      checkedItem: {}, //要操作的地址的某一个对象
    }
  },
  created() {
    this.getCartList()
  ...
  methods: {
    getCartList() {
      this.axios.get('/carts').then(res => {
        let list = res.cartProductVoList //获取购物车所有的商品
        this.cartList = list.filter(item => item.productSelected) //获取购物车选中的商品
    ...
    delAddress(item) { //删除地址按钮事件
      this.showDelModal = true
      this.checkedItem = item
    },
    editAddress(item) { //编辑地址按钮事件
      this.userAction = 0
      this.showEditModal = true
      this.checkedItem = item
    },
    addAddress(item) { //新增地址按钮事件
      this.userAction = 1
      this.showEditModal = true
      this.checkedItem = item
    },
    submitDel(checkedItem) { // 删除 提交事件
      this.showDelModal = false
      this.axios.delete(`/shippings/${checkedItem.id}`).then(() => {
        this.getAddressList()
        this.message('删除成功', 'success')
      })
    },
    submitEdit(checkedItem) { // 新增/修改 提交事件
      ...
      if (errMsg) {
        this.message(errMsg, 'warning')
        return;
      }
      let params = { receiverName, receiverMobile, receiverProvince, receiverCity, receiverDistrict, receiverAddress, receiverZip }
      if (this.userAction == 1) {
        this.axios.post('/shippings', params).then(() => {
          this.showEditModal = false
          this.getAddressList()
          this.message('新增地址成功', 'success')
        })
      } else {
        this.axios.put(`/shippings/${checkedItem.id}`, params).then(() => {
          this.showEditModal = false
          this.getAddressList()
          this.message('修改地址成功', 'success')
        })
      }
    },
    orderSubmit() { //订单提交
